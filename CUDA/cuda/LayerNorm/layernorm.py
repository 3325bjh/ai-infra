...
def _layernorm_forward(
    ...
    for offset in range(0,N,BLOCK_SIZE):
        ...
        x=tl.load(x_ptr+cols,mask=cols<N).to(tl.float32)
        sum_accumulator+=x
...
class LayerNorm(torch.autograd.Function):
    ...
    def forward(ctx,x,normalized_shape,weight,bias,eps):
        ...
        MAX_FUSED_SIEZ=65536//x.element_size()
        BLOCK_SIZE=min(MAX_FUSED_SIEZ,triton.next_power_of_2(N))
        if N>BLOCK_SIZE:
            raise RuntimeError("")
...
def _layernorm_backward_dLdx(
    ...
    c1=tl.sum(dydx_normed*x_normed,axis=0)/N
...
def _layernorm_backward_dLdw_dLdb(
    ...
    for i in range(0,GROUP_SIZE,BLOCK_SIZE_M):
        ...
        dLdw_acc+=tl.load(dLdw_inter_ptr+offsets,mask=mask)
        dLdb_acc+=tl.load(dLdb_inter_ptr+offsets,mask=mask)
...
def layernorm_kernel_test(M,N,dtype,eps=1e-5,device=DEVICE):
...
def benchmark(M,N,dtype,provider,mode="backward",eps=1e-5,device=DEVICE):
    ...
    dLdy=0.1*torch.randn_like(x)
    ms,min_ms,max_ms=triton.testing.do_bench(lambda: y.backward(dLdy,retain_graph=True),quantiles=quantiles,grad_to_none=[x],rep=500)
    return gbps(ms)


if __name__ == '__main__':
    layernorm_kernel_test(1151,8192,torch.float16)
