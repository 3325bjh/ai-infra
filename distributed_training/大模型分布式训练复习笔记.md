...
- [4. 数据并行、ZeRO 与 FSDP](#4-数据并行zero-与-fsdp)
  - [4.1 普通数据并行 DP](#41-普通数据并行-dp)
    - [4.1.3 大规模同步 DP：Large-Batch SGD](#413-大规模同步-dplarge-batch-sgd)
...
- [9. Mixture of Experts（MoE）与专家并行 EP](#9-mixture-of-expertsmoe与专家并行-ep)
  - [9.1 为什么使用 MoE](#91-为什么使用-moe)
  - [9.2 Router、top-k 与 routing map](#92-routertop-k-与-routing-map)
  - [9.3 TP、EP 与普通 Transformer 层的关系](#93-tpep-与普通-transformer-层的关系)
  - [9.4 四张 GPU 上的完整 dispatch 流程](#94-四张-gpu-上的完整-dispatch-流程)
  - [9.5 combine：expert 输出如何返回](#95-combineexpert-输出如何返回)
  - [9.6 专家负载不均衡问题](#96-专家负载不均衡问题)
  - [9.7 缓解专家偏斜的方法](#97-缓解专家偏斜的方法)
  - [9.8 MoE 的通信、显存与性能权衡](#98-moe-的通信显存与性能权衡)
- [10. Ring Attention](#10-ring-attention)
  - [10.1 序列切分](#101-序列切分)
  - [10.2 第一阶段：环形传递 Key](#102-第一阶段环形传递-key)
  - [10.3 第二阶段：环形传递 Value](#103-第二阶段环形传递-value)
  - [10.4 通信与显存特点](#104-通信与显存特点)
- [11. DistBelief、参数服务器与异步 SGD：历史背景](#11-distbelief参数服务器与异步-sgd历史背景)
  - [11.1 它要解决的时代问题](#111-它要解决的时代问题)
  - [11.2 DistBelief 的两层并行](#112-distbelief-的两层并行)
  - [11.3 参数服务器：共享参数的中心](#113-参数服务器共享参数的中心)
  - [11.4 Downpour SGD：异步更新流程](#114-downpour-sgd异步更新流程)
  - [11.5 异步的收益、代价与 Adagrad](#115-异步的收益代价与-adagrad)
...
#### 4.1.3 大规模同步 DP：Large-Batch SGD

普通 DP 增加 GPU 数时，若要保持每张 GPU 的计算量不变，必须保持本地 batch（local batch）$n$ 不变，并将全局 batch（global batch）随 worker 数 $k$ 线性扩大：

$$B_{\mathrm{global}}=kn$$

例如从 8 张 GPU、每卡 32 个样本扩展到 256 张 GPU 时，global batch 从 $256$ 增至 $8192$，而每卡仍处理 32 个样本。Large Mini-Batch SGD 论文表明：对 ImageNet 上的 ResNet-50，只要正确处理优化和实现细节，这种扩展可保持与小 batch 相近的精度。

##### 线性学习率缩放

设小 batch 基线的学习率为 $\eta$；当 global batch 扩大为原来的 $k$ 倍时，采用：

$$\hat\eta=k\eta$$

直觉是：连续执行 $k$ 次小 batch 更新，与合并这 $k$ 个 batch 做一次大 batch 更新相比，若这段时间内参数变化不大，则有：

$$\nabla l(x,w_t)\approx\nabla l(x,w_{t+j}),\qquad j<k$$

此时将学习率同步乘以 $k$，一次大 batch 更新就近似等于这 $k$ 次小 batch 更新的总效果。实践中，这条规则通常不仅能匹配最终精度，也能使以“已处理样本数”为横轴的训练曲线接近。

##### 渐进式 warmup

训练初期参数变化很快，上述近似不成立；若一开始就使用 $k\eta$，大 batch 训练容易不稳定。解决方法是 warmup：从小 batch 基线学习率 $\eta$ 开始，在前若干个 epoch 内逐迭代线性增加到目标学习率 $k\eta$。
...
相比“前几轮固定使用低学习率、随后突然跳至 $k\eta$”的恒定 warmup，渐进式 warmup 避免了突变，特别适合很大的 batch。warmup 时长不是高度敏感的超参数；论文中使用 5 个 epoch。

##### 容易出错的实现细节

1. **固定 local batch，尤其是 BatchNorm 的统计 batch。** BN 的均值和方差依赖参与统计的样本数。扩展 GPU 数时，应保持每卡 local batch（论文中为 32）不变，并在每张卡内独立计算 BN；不要跨所有 worker 做 SyncBN。否则 BN 的统计噪声和实际优化目标都会改变。

2. **梯度应按 global batch 平均。** 每卡 local batch 为 $n$、共 $k$ 卡时，所需梯度为：

   $$g=\frac{1}{kn}\sum_{j=1}^{k}\sum_{x\in B_j}\nabla l(x,w)$$

   若 all-reduce 只做求和，则每卡 loss 应除以 $kn$，或在梯度聚合后除以 $k$。不要误按 $n$ 归一化后又直接使用求和结果。

3. **真正缩放 optimizer 的学习率。** 将交叉熵 loss 乘以 $k$ 不等价于学习率乘以 $k$，因为 weight decay 对应的 $\lambda w$ 项不会随交叉熵一起缩放，正则化强度会被意外改变。

4. **学习率改变时做动量修正。** 若实现将学习率吸收到动量缓冲 $v$ 中，且 $v_t=\eta_tu_t$，学习率从 $\eta_t$ 变为 $\eta_{t+1}$ 时，历史项应乘以 $\eta_{t+1}/\eta_t$：

   $$v_{t+1}=m\frac{\eta_{t+1}}{\eta_t}v_t+\eta_{t+1}g_t$$

   warmup 期间学习率持续上升，不做此修正会使历史动量相对过小。

5. **所有 worker 共享一次 epoch 级 shuffle。** 每个 epoch 先对完整训练集随机打乱一次，再将该排列切给不同 worker，确保每个样本在该 epoch 中恰好处理一次；不要让各 worker 独立随机抽取而引入重复或遗漏。

通信侧还应将已完成层的梯度尽早发起 all-reduce，使其与前面层的反向计算重叠；具体 all-reduce 算法见[第 2.1 节](#21-nccl-通信原语)。
...
为缓解上述显存瓶颈，GPipe 支持 **激活重计算**（也称 rematerialization 或 activation checkpointing）。前向传播时，每个 accelerator 不再保存 partition 内每一层的中间激活，而只保存 partition 边界处的输出激活。反向传播需要某个 partition 的内部激活时，再从边界输入重新执行该 partition 的复合前向函数 $F_k$，生成反向计算所需的中间结果：
...
反向：
读取 partition 边界输入
→ 重新执行该 partition 的前向函数 F_k
→ 使用重算得到的激活执行反向传播
→ 释放临时激活
```

为避免与 GPipe 原文符号混淆，这里设 mini-batch 总大小为 $B$，切分为 $m$ 个 micro-batch，因而单个 micro-batch 的样本数为 $b=B/m$；流水线有 $p$ 个 stage，模型共有 $L$ 层，则每个 stage 平均包含 $L/p$ 层。激活重计算后，单卡长期保存的激活主要是 partition 边界处的激活和当前 partition 的检查点，因此峰值激活显存可近似写为：

$$M_{\mathrm{activation}}^{\mathrm{remat}}=O\left(B+\frac{L}{p}\times b\right)=O\left(B+\frac{L}{p}\times\frac{B}{m}\right)$$

这里第一项 $B$ 对应 mini-batch 规模，第二项对应一个 partition 的层数和单个 micro-batch 的激活规模。相比之下，如果不做重计算和 partition，论文给出的激活显存量级为 $O(B\times L)$；若只比较已经 partition 的 GPipe 且不做重计算，则需要为全部 $m$ 个 micro-batch 保存 partition 内激活，显存会随 $m$ 近似线性增长：

$$M_{\mathrm{activation}}^{\mathrm{no\ remat}}=O\left(m\times\frac{L}{p}\times b\right)$$

因此，激活重计算用额外的前向计算换取显著的激活显存下降。需要注意，重计算并不会消除模型参数、梯度和通信 buffer 的显存开销；它主要减少的是需要跨越前向和反向阶段长期保存的中间激活。

GPipe 的流水线气泡开销在 $m$ 个 micro-step 上摊销后，量级约为 $O((p-1)/m)$。当 $m\ge 4p$ 时，实验中气泡开销通常已经较小；同时，反向阶段的 partition 重计算可以尽早调度，部分隐藏等待更早层梯度的时间。

GPipe 的通信开销也较低：stage 之间只需在 partition 边界传递激活和激活梯度，而不需要交换 partition 内部的中间激活。因此，即使 accelerator 之间缺少高速互联，也能保持较好的扩展效率。不过，实际模型的不同层在显存需求和计算量上可能并不均衡；如果 partition 划分不均匀，最慢的 stage 会造成负载不平衡，抵消部分流水线并行收益。
...
## 9. Mixture of Experts（MoE）与专家并行 EP

### 9.1 为什么使用 MoE

MoE（Mixture of Experts，混合专家）把一个普通 Transformer 的稠密 FFN 替换成多个专家 FFN，并让 Router 为每个 token 选择少数几个专家。这样可以在基本保持每个 token 计算量可控的同时，显著增加模型的总参数量。
...
若有 `E` 个专家、每个 token 选择 `k` 个专家，则 Router 通常产生形状为 `[num_tokens, E]` 的 logits，经过 softmax 后取 top-k。每个专家本身一般是一个两层 FFN：

```text
Expert_i(x) = W2_i · activation(W1_i · x)
```

注意，MoE 的“稀疏”通常指 token 只激活少数专家，不代表通信也是稀疏的。token 需要被发送到拥有目标专家的 GPU，因此 EP（Expert Parallel，专家并行）会引入 All-to-All 通信。

### 9.2 Router、top-k 与 routing map

Router 对每个 token 计算专家 logits。例如有 4 个 token、4 个专家、`top-k=2`：

```text
token       E0    E1    E2    E3       选中的专家
t0         0.8   0.1   0.7   0.2       E0、E2
t1         0.2   0.9   0.3   0.4       E1、E3
t2         0.1   0.2   0.6   0.5       E2、E3
t3         0.7   0.3   0.2   0.4       E0、E3
```

在实现中常见两个相关张量：

- `routing_map`：布尔或 0/1 张量，形状通常为 `[num_tokens, num_experts]`；`routing_map[t, e] = 1` 表示 token `t` 被送往专家 `e`。
- `probs`：与选中路由对应的门控权重。一个 token 被多个专家处理后，专家输出要乘以对应概率再相加。
...
`num_tokens` 是实际参与路由的 token 数，不是 `top-k`。但路由后的 token 副本数量大致是 `num_tokens × top-k`，因为一个 token 可能同时发送给多个专家。比如 `num_tokens=4`、`top-k=2` 时，最多产生 8 条 token-expert 路由记录。

随后通常进行第一次 permutation：按照目标专家或目标 EP rank 对 token 重排，使发往同一目的地的 token 连续存放。这样可以通过 split size 描述每个目的地要接收多少 token，并高效调用 All-to-All。

### 9.3 TP、EP 与普通 Transformer 层的关系

这里需要区分四个容易混淆的概念：

- **TP（Tensor Parallel）**：对普通 Transformer 的线性层、Attention 和稠密 FFN 做张量切分。同一个逻辑层的权重矩阵被多个 TP rank 共同持有并协同计算。
- **EP（Expert Parallel）**：沿“专家编号”切分专家集合。不同 EP 分区负责不同的专家，因此 token 需要通过 EP 通信到达目标专家所在的分区。
- **ETP（Expert Tensor Parallel）**：对“单个专家内部”的 FFN 再做张量切分。一个专家的 `W1/W2` 不一定完整放在一个 GPU 上，而是由一个 ETP group 共同持有和计算。配置项通常是 `expert_tensor_parallel_size`，在未单独设置时通常回退到普通 `tensor_model_parallel_size`。
- **SP（Sequence Parallel）**：主要是 activation 的布局方式，沿 token/sequence 维度分布激活，常与 TP 配合以降低激活显存；它不是另一种专家分配策略。

因此，MoE 中更准确的关系是：

```text
EP 负责：不同专家放在哪个专家分区
ETP 负责：一个专家的权重如何在分区内部切分
TP 负责：普通层的权重如何切分
SP 负责：activation 的 token 维度如何分布
```

#### 四张 GPU 的拓扑例子

假设 `world_size=4`、`TP=2`、`EP=2`、`ETP=2`、`PP=1`、`CP=1`，有 4 个专家 E0～E3。按照当前 `initialize_model_parallel` 的默认 `order="tp-cp-ep-dp-pp"`，普通层和专家层会分别建立如下通信组：
...
普通层的 DP group：
DP group 0 = [GPU0, GPU2]
DP group 1 = [GPU1, GPU3]
...
专家 EP group（同一 ETP 分片位置之间通信）：
EP group 0 = [GPU0, GPU2]
EP group 1 = [GPU1, GPU3]
```

这个例子中，E0/E1 属于一个 EP 分区，E2/E3 属于另一个 EP 分区；但 E0 并不是只存在于 GPU0 上，而是由 GPU0、GPU1 组成的 ETP group 共同保存。类似地，E2 由 GPU2、GPU3 共同保存。GPU0 和 GPU2 之所以在同一个 EP group，是因为它们分别持有不同专家的“相同 ETP 分片位置”，需要沿 EP 维度交换 token。

可以用二维坐标理解每张卡：

```text
              ETP rank 0       ETP rank 1
EP 分区 0        GPU0              GPU1
EP 分区 1        GPU2              GPU3
```

这里的“EP 分区”决定专家集合，“ETP rank”决定该专家的哪一块权重。注意，`world_size=4、TP=2、EP=2` 时，普通层的 `data_parallel_size` 是 `4/2=2`；专家侧的 `expert_data_parallel_size` 是 `4/(ETP×EP)=1`。因此 GPU0/GPU2 是普通层的两个 DP 副本，同时也是专家 EP group 的成员，但 DP 和 EP 在语义上不是同一个并行维度。实际 Megatron 的 rank 编号和通信组排列由 `initialize_model_parallel` 根据配置和 `order` 创建，具体顺序改变时组内 rank 可能不同。

#### 普通层为什么不需要 EP

普通 Transformer 层没有 E0、E1 这样的专家选择，也不会执行 router 和 EP All-to-All。普通层只使用普通 TP group（以及可能存在的 DP、PP、SP）完成计算。EP 维度上的多份 rank 通常保存普通层参数的副本，或承载不同的 activation/数据分片；它们不是被“闲置”，而是 EP 只不参与该层的专家路由。

进入 MoE 层后，普通层输出的 hidden states 先在各自 rank 上经过 router。之后，同一组 GPU 使用已建立的 EP group 交换 token；交换完成后，每个 ETP group 才能拿到属于自己专家的 token，并协同执行专家 FFN。因此，普通层到 MoE 层不是重新创建 GPU，而是同一批 rank 在不同层使用不同的通信组和参数布局。

### 9.4 四张 GPU 上的完整 dispatch 流程

下面继续使用 `TP=2、EP=2、ETP=2` 的四卡例子。这里不能简单说“每个 EP rank 拥有两个完整专家”，更准确的说法是：

```text
EP 分区 0 的专家集合：E0、E1，由 GPU0/GPU1 以 ETP=2 共同保存
EP 分区 1 的专家集合：E2、E3，由 GPU2/GPU3 以 ETP=2 共同保存
```

假设启用 Sequence Parallel，注意力输出经过 TP group 内的 Reduce-Scatter 后，每张 GPU 得到不同的 sequence token。Router 对本地 token 做 `top-k=2`，得到如下路由：
...
#### 从每张 GPU 的初始 token 开始跟踪

下面严格按照上述代码配置说明一个实际运行布局。由于普通层的 `data_parallel_size=2`，两个 DP 副本分别处理不同的数据；启用 SP 后，每个 TP rank 还持有本 DP 副本的一段 sequence。于是四张 GPU 上的初始 token 可以写成：

```text
GPU0（DP0，TP0）：t0、t1 的完整 hidden activation
GPU1（DP0，TP1）：t2、t3 的完整 hidden activation
GPU2（DP1，TP0）：t4、t5 的完整 hidden activation
GPU3（DP1，TP1）：t6、t7 的完整 hidden activation
```

这里 token 在每张卡上不同，是 SP 和 DP 共同造成的：GPU0/GPU1 属于同一个 DP 副本，但沿 sequence 维度分片；GPU2/GPU3 是另一个 DP 副本，也沿 sequence 维度分片。每张卡上的 activation 仍然是完整 hidden size，切分的是 token 位置：

```text
TP group A：[GPU0, GPU1] → DP0 的 t0、t1、t2、t3
TP group B：[GPU2, GPU3] → DP1 的 t4、t5、t6、t7
```

如果关闭 SP，则同一个 TP group 内的 rank 通常处理同一组 sequence token，只是参与 TP 计算；此时不能期待四张 GPU 初始 token 都不同。下面选择启用 SP，是为了展示真实代码中“每个 rank 初始 token 不同”的情况。

```text
token   专家 1(prob)   专家 2(prob)
t0      E0 (0.60)      E2 (0.40)
t1      E1 (0.70)      E3 (0.30)
t2      E0 (0.20)      E3 (0.80)
t3      E1 (0.40)      E2 (0.60)
...
所以 `t0` 会产生 `(t0,E0)` 和 `(t0,E2)` 两条路由，但它们都带有 t0 的信息；最后必须把两条结果按照 0.60 和 0.40 合并。

#### 每张 GPU 执行 Permutation 1

在 `EP=2`、`ETP=2` 下，可以把专家布局理解为：

```text
EP 分区 0：E0、E1，由 GPU0/GPU1 组成的 ETP group 计算
EP 分区 1：E2、E3，由 GPU2/GPU3 组成的 ETP group 计算
```

Permutation 1 根据目标专家，把每张卡上的 token-expert 副本分成两类。只列 token ID，实际数据还包含完整 hidden activation、prob 和 restore mapping：
...
GPU3：
  EP0：t7→E0
  EP1：t6→E1、t6→E2、t7→E3
```

例如，GPU0 上的 `t0` activation 会形成两个副本：一个发往 E0，一个发往 E2。GPU1 上没有 t0，而是处理自己的 t2、t3。这里的 token 差异来自 SP 的 sequence 切分；TP/ETP 仍然负责权重和 GEMM 的张量切分。

按专家统计，当前 batch 的逻辑 token 数为：

```text
E0：t0、t2、t5、t7，共 4 个
E1：t1、t3、t5、t6，共 4 个
E2：t0、t3、t4、t6，共 4 个
E3：t1、t2、t4、t7，共 4 个
```

实际代码还要按照 rank 和通信组统计 `tokens_per_expert`、`input_splits` 与 `output_splits`。如果不同专家收到的 token 数不同，这些 split size 也会不同。

#### EP All-to-All 之后每张 GPU 上的 token

EP All-to-All 使用 EP 方向的通信组，把 EP0 的路由送到 E0/E1 所在的 ETP group，把 EP1 的路由送到 E2/E3 所在的 ETP group。通信完成后，逻辑上的目标布局是：

```text
GPU0（E0/E1 的 ETP rank 0）：
  来自 GPU0 的 t0→E0、t1→E1
  来自 GPU2 的 t5→E0、t5→E1

GPU1（E0/E1 的 ETP rank 1）：
  来自 GPU1 的 t2→E0、t3→E1
  来自 GPU3 的 t7→E0、t6→E1

GPU2（E2/E3 的 ETP rank 0）：
  来自 GPU0 的 t0→E2、t1→E3
  来自 GPU1 的 t3→E2、t2→E3
  来自 GPU2 的 t4→E2、t4→E3
  来自 GPU3 的 t6→E2、t7→E3

GPU3（E2/E3 的 ETP rank 1）：
  来自 GPU0 的 t0→E2、t1→E3
  来自 GPU1 的 t3→E2、t2→E3
  来自 GPU2 的 t4→E2、t4→E3
  来自 GPU3 的 t6→E2、t7→E3
...
在这一步要区分“EP All-to-All 刚接收的 buffer”和“后续 TP gather 后的输入”。上表是每个 EP rank 的实际接收结果：GPU0/GPU2 属于 `[GPU0,GPU2]`，GPU1/GPU3 属于 `[GPU1,GPU3]`。`dispatch_postprocess` 随后通过 `gather_from_sequence_parallel_region` 在 ETP/TP group 内沿第一维收集 token，使同一专家的两个 ETP rank 都能看到完整的本地专家 token 集合。top-k=2 使每个 token 产生两份 token-expert 副本，但不是把它当成两个独立 token。

#### Permutation 2：在目标 ETP group 内按专家分组

All-to-All 接收后，数据可能仍按来源 rank 排列，不能直接把整个 buffer 当作一个专家的输入。Permutation 2 会按照本地专家重新排序：

```text
GPU0/GPU1 的 ETP group：
  E0 输入：t0、t2、t5、t7
  E1 输入：t1、t3、t5、t6

GPU2/GPU3 的 ETP group：
  E2 输入：t0、t3、t4、t6
  E3 输入：t1、t2、t4、t7
```
...
```text
E0：GPU0/GPU1 协同处理 t0、t5、t2、t7
E1：GPU0/GPU1 协同处理 t1、t5、t3、t6
E2：GPU2/GPU3 协同处理 t0、t3、t4、t6
E3：GPU2/GPU3 协同处理 t1、t2、t4、t7
```

### 9.5 combine：expert 输出如何返回

专家 FFN 完成后，输出仍然按照 Permutation 2 形成的“专家分组布局”排列。`combine` 不是直接把输出相加，而是严格按照 dispatch 的逆过程，将每条 token-expert 输出送回原始 token 所在的 rank，再恢复 token 顺序并完成 top-k 加权。

```text
专家 FFN 输出（仍按本地专家分组）
    ↓ 撤销 Permutation 2
恢复各个来源 rank 对应的 chunk 顺序
    ↓ TP Reduce-Scatter（TP>1 时）
恢复 sequence-parallel 的 token 分片
    ↓ 第二次 EP All-to-All
把每条专家输出发回原始 token 所在 rank
    ↓ 撤销 Permutation 1
恢复 token 原顺序，并按 probs 合并 top-k 专家输出
    ↓
MoE 输出
```

#### 9.5.1 `combine_preprocess`：先撤销 Permutation 2

专家计算输入在 dispatch 后已经按本地专家排列，例如：

```text
[E0 的全部输出][E1 的全部输出]...
```

但是，第二次 EP All-to-All 需要按照原来的来源 rank 和 EP 通信边界发送数据，而不是按照 E0、E1 的顺序发送。因此 `combine_preprocess` 使用 `restore_output_by_local_experts` 和 `num_global_tokens_per_local_expert.T.ravel()`，把专家分组布局重新排列为通信 chunk 布局。

这一步就是撤销 Permutation 2，发生在第二次 EP All-to-All **之前**：
...
如果启用了 `drop_and_pad`，代码使用 `view → transpose → flatten` 完成同样的逆排列；由于每个专家都有固定 `capacity`，不需要按照实际 token 数进行不等长排序。

#### 9.5.2 TP Reduce-Scatter：恢复 sequence-parallel 布局

Permutation 2 撤销后，如果 `tp_size > 1`，代码继续调用：
...
这里的 Reduce-Scatter 有两个作用：

1. 对 TP group 内各 rank 的部分结果进行求和；
2. 沿第一维重新切分 token，使每个 rank 回到 sequence-parallel 的局部 token 布局。

它不是撤销 Permutation 2，也不是 EP 通信。ETP/TP 规约负责张量并行计算结果，EP All-to-All 负责跨专家分区搬运 token-expert 输出。

#### 9.5.3 `token_combine`：第二次 EP All-to-All

完成 `combine_preprocess` 后，才进入 `token_combine`：
...
```text
all_to_all(ep_group, input, output_splits, input_splits)
```
...
原因是通信方向反过来了：dispatch 的 `input_splits` 描述本 rank 发出的数量、`output_splits` 描述本 rank 收到的数量；combine 返回时，原来的发送量和接收量互换。此时每条专家输出会回到产生对应 token 副本的 rank。

#### 9.5.4 `combine_postprocess`：撤销 Permutation 1 并合并

第二次 EP All-to-All 完成后，输出已经回到了原始 token 所在 rank，但仍可能按照 token-expert 路由顺序排列。`combine_postprocess` 使用 `reversed_local_input_permutation_mapping` 调用 `unpermute`，撤销第一次 Permutation：
...
`unpermute` 根据 `routing_map` 和 `probs` 找到同一个原始 token 的多个专家结果，并进行加权求和，而不是简单拼接。例如 9.4 中：

```text
t0 → E0 (0.60)、E2 (0.40)

y(t0) = 0.60 × E0(t0) + 0.40 × E2(t0)
```
...
若启用了 shared experts，共享专家可以与 routed experts 并行或重叠执行。实现中的 stream wait、异步 All-to-All 等代码主要是在保证依赖正确的前提下，让通信与 shared expert 的 GEMM 重叠，减少通信暴露时间。

### 9.6 专家负载不均衡问题

Router 可能长期偏爱某几个专家。例如 8 个专家中，大量 token 都选择 E0、E1，而 E6、E7 几乎没有输入。这会造成：

1. 热门专家的 GEMM 排队或超出 capacity，产生 token dropping 或 padding 浪费。
2. 不同 EP rank 接收的 token 数差异很大，All-to-All 被最慢的 rank 拖住。
3. 专家参数利用率不均，部分专家训练不足，路由进一步恶化。
4. 负载不均造成显存峰值和通信峰值，吞吐下降。

需要区分两件事：auxiliary loss 主要改变 Router 的训练目标；capacity 和 token dropping 主要限制一次前向的最坏负载；expert bias 和 Sinkhorn 等方法直接改变路由决策。实际系统通常组合使用多种方法。

### 9.7 缓解专家偏斜的方法

#### 1. Auxiliary load-balancing loss

统计一个 batch 中每个专家接收的 token 比例 `f_e`，以及 Router 分给该专家的平均概率 `p_e`，加入类似 Switch Transformer 的负载均衡损失：

```text
L_aux = α · E · Σ_e (f_e · p_e)
```

当少数专家同时拥有较高的 token 比例和路由概率时，损失会增大，反向传播会推动 Router 分散选择。它实现简单、训练稳定，但依赖统计窗口：batch 太小或只在局部范围统计时，均衡信号可能噪声较大。

#### 2. Expert bias / 动态专家偏置

为每个专家维护一个不参与梯度更新的 bias。某专家超载时降低其 bias，某专家空闲时提高其 bias，再将 bias 加到 Router logits 后做 top-k。这样可以直接把新 token 引导到空闲专家。bias 更新需要谨慎设置步长，否则可能在专家之间来回振荡。

#### 3. Quantile balancing 与容量约束

可以按照专家的分数分布或分位点设定选择阈值，使每个专家接收数量更接近目标容量。常见的 capacity factor 是：

```text
capacity ≈ capacity_factor × (num_tokens × top-k / num_experts)
```

超过 capacity 的路由可以丢弃、回退到残差分支，或采用 padding 固定形状。较大的 capacity 减少丢 token，但增加显存和计算；较小的 capacity 节省资源，却可能损伤模型质量。

#### 4. Sinkhorn / balanced assignment

把一批 token 到专家的路由看成带有行列约束的分配问题，通过 Sinkhorn 迭代让 token 选择和专家容量同时接近平衡。这种方法能从路由阶段更强地控制均衡，但需要额外计算，并可能限制 Router 的自由度。

#### 5. Group-limited top-k 与层级路由

先在专家组之间选择少数组，再只在选中的组内选专家，可以限制跨组通信范围，减少某一通信域被打爆。它既能控制 EP 通信，也能通过组级别的负载约束缓解局部热点，但可能牺牲全局最优专家选择。

#### 6. 增大统计范围并改善训练策略

使用更大的 global batch、跨 DP/EP rank 聚合统计、合理初始化专家和 Router、设置 Router z-loss 防止 logits 过大，都有助于避免早期塌缩到少数专家。需要注意，局部均衡不等于全局均衡：每个 rank 看起来均匀时，跨 rank 的专家总负载仍可能不均。

#### 7. 系统层面的缓解

即使路由无法完全均匀，也可以通过动态 capacity、token dropping、通信与 GEMM 重叠、合理安排 EP group、按专家负载调度 kernel 等方式减少性能损失。它们不能替代路由层面的均衡损失，但能降低热点带来的尾延迟。

### 9.8 MoE 的通信、显存与性能权衡

MoE 的优势是总参数量大而单 token 激活专家少；代价是 token dispatch/combine 的通信和复杂的动态形状管理。分析性能时应同时看：

```text
计算量：每个 token 实际激活的专家数 top-k
参数量：专家数 × 单专家参数量
通信量：EP All-to-All 的 token 副本数量
显存量：本地专家权重 + dispatch/receive buffer + capacity padding
负载：各专家及各 EP rank 的 token 分布
```

一个实用的检查顺序是：先确认 `routing_map` 和 `tokens_per_expert` 是否正确，再确认 dispatch 后每个专家收到的 token 是否与 split size 一致，最后检查 combine 是否按逆序恢复 token。若只看最终 loss，很难区分是 Router 负载问题、EP 通信问题，还是 permutation/restore mapping 出错。
